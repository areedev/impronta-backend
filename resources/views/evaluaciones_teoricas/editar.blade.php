...
<div class="widget-content searchable-container list">
    ...
    {!! Form::open([
    'route' => ['evaluaciones.teorica.update', $evaluacion->id],
    'method' => 'PUT',
    ...
    <div class="card card-body">
        <div class="row">
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    {{ Form::number('preguntas', $teorica->preguntas, ['class' => 'form-control', 'required']) }}
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                    {{ Form::label('preguntas_buenas', 'Preguntas buenas', ['class' => 'form-label']) }}
                    {{ Form::number('preguntas_buenas', $teorica->preguntas_buenas, ['class' => 'form-control', 'required']) }}
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                    {{ Form::label('archivo', 'Subir archivo', ['class' => 'form-label']) }} | <a class="badge bg-primary text-white capitalize inline-flex items-center" href="{{asset('uploads/evaluacion/'.$evaluacion->id.'/teorica/'.$teorica->archivo.'')}}" target="_blank">Descargar Archivo</a>
        ...
        <div class="row preguntascontenedor">
            @foreach ($teorica->items as $item)
            <div class="col-md-12 pregunta">
                <div class="row d-flex">
                    <div class="col-md-1">
                        {{ Form::label('tipo['.$item->id.']', 'Tipo', ['class' => 'form-label']) }}
                        <div class="mb-2">
                            {{ Form::select('tipo['.$item->id.']', $tipos, $item->competencia->tipo_competencia_id, ['class' => 'select2 tipo', 'placeholder' => 'Seleccionar', 'required']) }}
                        </div>
                    </div>
                    <div class="col-md-5">
                        {{ Form::label('competencia['.$item->id.']', 'Item', ['class' => 'form-label']) }}
                        <div class="mb-3">
                            {{ Form::select('competencia['.$item->id.']', [$item->competencia_id => $item->competencia->nombre], $item->competencia_id, ['class' => 'select2 competencia', 'placeholder' => 'Seleccionar', 'required']) }}
                        </div>
                    </div>
                    <div class="col-md-4">
                        {{ Form::label('pregunta['.$item->id.']', 'Pregunta', ['class' => 'form-label']) }}
                        {{ Form::text('pregunta['.$item->id.']', $item->pregunta, ['class' => 'form-control', 'placeholder' => '', 'required']) }}
                    </div>
                    <div class="col-md-1">
                        <div class="mb-2">
            ...
            <div class="d-flex justify-content-center">
                <div class="col-md-4 text-center">
                    <div class="mb-3">
                        ...
                        {{ Form::number('nota', $teorica->nota, ['class' => 'form-control', 'required','min' => 0, 'max' => 4, 'step' => '0.1']) }}
                    </div>
                </div>
            </div>
        </div>
