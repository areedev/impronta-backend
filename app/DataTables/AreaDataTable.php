    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('acciones', function (Area $area) {
                return view('area.acciones', compact('area'));
            })
            ->addColumn('created_at', function (Area $area) {
                return $area->created_at->format('d-m-Y h:i');
            })
            ->addColumn('usuario_creador', function (Area $area) {
                return ($area->creador) ? $area->creador->nombre . ' ' . $area->creador->apellido : 'No Encotrado';
            })
            ->rawColumns(['acciones'])
            ->setRowId('id');
    }
